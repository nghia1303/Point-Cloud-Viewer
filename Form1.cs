...
namespace PointCloudViewer
{
    ...
    public partial class Form1 : Form
    {
        ...
        private bool _isFileOpening = false;
        private bool _isInitialOpenGL = false;
        private int _pointSize = 2;
        private double _transX = 0;
        private double _transY = 0;
        private double _angleX = 0;
        private double _angleY = 0;
        private float _scaling = 1.0f;
        private Vector3d _minV;
        private Vector3d _maxV;
        private string _primitiveObject = "las";
        private bool _isShowOctreeOutline = false;
        private double[,] _frustum = new double[6, 4];
        private bool _isLeftMouseButtonPushed = false;
        private bool _isRightMouseButtonPushed = false;
        ...
        private bool _hasLas = false;
        private bool _isCuttingPoint = false;
        private Vector3d _mousePosition = Vector3d.Zero;
        private List<ColorPoint> _points;
        private Point3DExt _pivotPoint;
        private List<Vector3d> _vertices;
        private ResourceManager _resourceManager;
        private PointCloudColorMapper _colorMapper;
        ...
        private Matrix4d _modelViewMatrix = Matrix4d.Identity;
        private Matrix4d _modelMatrix = Matrix4d.Identity;
        private float _currentYaw = 0;
        private float _currentPitch = 0;
        private float _previousYaw = 0;
        private float _previousPitch = 0;
        private float _offsetYaw = 0;
        private float _offsetPitch = 0;

        private float _currentTransX = 0;
        private float _currentTransY = 0;
        private float _previousTransX = 0;
        ...
        private void Render(int pointSize, bool ShowOctreeOutline, string pointCloudColor)
        {
            ...
            _currentYaw = _previousYaw + _offsetYaw;
            _currentPitch = _previousPitch + _offsetPitch;
            _currentTransX = _previousTransX + offsetTransX;
            _currentTransY = previousTransY + offsetTransY;

            GL.LoadMatrix(ref _modelViewMatrix);
            Vector4d transformPivot = Vector4d.Transform(new Vector4d(_pivotPoint.Point, 1.0), _modelMatrix);
            Vector3d newPivot = new Vector3d(transformPivot.X, transformPivot.Y, transformPivot.Z);
            Matrix4d rotationMatrix = Matrix4d.CreateRotationX(MathHelper.DegreesToRadians(-_offsetPitch)) *
                                      Matrix4d.CreateRotationY(MathHelper.DegreesToRadians(_offsetYaw));
            ...
            _modelMatrix *= Matrix4d.CreateTranslation(offsetTransX, offsetTransY, 0);
            _modelViewMatrix = _modelMatrix;

            _previousPitch = _currentPitch;
            _previousYaw = _currentYaw;
            _previousTransX = _currentTransX;
            previousTransY = _currentTransY;

            CalculateFrustum();
            if (_pco != null)
            {
                SetupViewport();
                _pco.Render(pointSize, ShowOctreeOutline, pointCloudColor, _frustum);
            }
            if (_pivotPoint.Point != Vector3d.Zero && _pivotPoint.Flag != 10000)
            {
            ...
            if (cb_CutPoint.Checked)
            {
                ...
                for (int i = 0; i < _vertices.Count; i++)
                {
                    if (i > 0)
                    {
                        ...
                        GL.Vertex3(_vertices[i - 1]);
                        GL.End();
                    }
                    if (i < 2 && _vertices.Count <= 3)
                    {
                        GL.Begin(PrimitiveType.Lines);
                        GL.Color3(Color.Yellow);
                        GL.Vertex3(_vertices[_vertices.Count - 1]);
                        GL.Vertex3(_mousePosition);
                        GL.End();
                    }
                    if (i == 2 && _vertices.Count <= 3)
                    {
        ...
        public void CalculateFrustum()
        {
            ...
            clipMatrix[0] = (modelViewMatrix.M11 * projectionMatrix.M11)
            ...
            clipMatrix[1] = (modelViewMatrix.M11 * projectionMatrix.M12)
            ...
            clipMatrix[2] = (modelViewMatrix.M11 * projectionMatrix.M13)
            ...
            clipMatrix[3] = (modelViewMatrix.M11 * projectionMatrix.M14)
            ...
            clipMatrix[4] = (modelViewMatrix.M21 * projectionMatrix.M11)
            ...
            clipMatrix[5] = (modelViewMatrix.M21 * projectionMatrix.M12)
            ...
            clipMatrix[6] = (modelViewMatrix.M21 * projectionMatrix.M13)
            ...
            clipMatrix[7] = (modelViewMatrix.M21 * projectionMatrix.M14)
            ...
            clipMatrix[8] = (modelViewMatrix.M31 * projectionMatrix.M11)
            ...
            clipMatrix[9] = (modelViewMatrix.M31 * projectionMatrix.M12)
            ...
            clipMatrix[10] = (modelViewMatrix.M31 * projectionMatrix.M13)
            ...
            clipMatrix[11] = (modelViewMatrix.M31 * projectionMatrix.M14)
            ...
            clipMatrix[12] = (modelViewMatrix.M41 * projectionMatrix.M11)
            ...
            clipMatrix[13] = (modelViewMatrix.M41 * projectionMatrix.M12)
            ...
            clipMatrix[14] = (modelViewMatrix.M41 * projectionMatrix.M13)
            ...
            clipMatrix[15] = (modelViewMatrix.M41 * projectionMatrix.M14)
            ...
            _frustum[RIGHT, 0] = clipMatrix[3] - clipMatrix[0];
            _frustum[RIGHT, 1] = clipMatrix[7] - clipMatrix[4];
            _frustum[RIGHT, 2] = clipMatrix[11] - clipMatrix[8];
            _frustum[RIGHT, 3] = clipMatrix[15] - clipMatrix[12];
            NormalizePlane(_frustum, RIGHT);

            _frustum[LEFT, 0] = clipMatrix[3] + clipMatrix[0];
            _frustum[LEFT, 1] = clipMatrix[7] + clipMatrix[4];
            _frustum[LEFT, 2] = clipMatrix[11] + clipMatrix[8];
            _frustum[LEFT, 3] = clipMatrix[15] + clipMatrix[12];
            NormalizePlane(_frustum, LEFT);

            _frustum[BOTTOM, 0] = clipMatrix[3] + clipMatrix[1];
            _frustum[BOTTOM, 1] = clipMatrix[7] + clipMatrix[5];
            _frustum[BOTTOM, 2] = clipMatrix[11] + clipMatrix[9];
            _frustum[BOTTOM, 3] = clipMatrix[15] + clipMatrix[13];
            NormalizePlane(_frustum, BOTTOM);

            _frustum[TOP, 0] = clipMatrix[3] - clipMatrix[1];
            _frustum[TOP, 1] = clipMatrix[7] - clipMatrix[5];
            _frustum[TOP, 2] = clipMatrix[11] - clipMatrix[9];
            _frustum[TOP, 3] = clipMatrix[15] - clipMatrix[13];
            NormalizePlane(_frustum, TOP);

            _frustum[BACK, 0] = clipMatrix[3] - clipMatrix[2];
            _frustum[BACK, 1] = clipMatrix[7] - clipMatrix[6];
            _frustum[BACK, 2] = clipMatrix[11] - clipMatrix[10];
            _frustum[BACK, 3] = clipMatrix[15] - clipMatrix[14];
            NormalizePlane(_frustum, BACK);

            _frustum[FRONT, 0] = clipMatrix[3] + clipMatrix[2];
            _frustum[FRONT, 1] = clipMatrix[7] + clipMatrix[6];
            _frustum[FRONT, 2] = clipMatrix[11] + clipMatrix[10];
            _frustum[FRONT, 3] = clipMatrix[15] + clipMatrix[14];
        ...
        private void glControl1_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            {
                _isLeftMouseButtonPushed = true;
                _leftMouseButtonPosition = e.Location;
            }
            else if (e.Button == MouseButtons.Right)
            {
                _isRightMouseButtonPushed = true;
                _rightMouseButtonPosition = e.Location;
                ...
                _pivotPoint = new Point3DExt();
                _pivotPoint.Flag = 10000;

                _pco.FindClosestPoint(_frustum, nearPoint, farPoint, ref _pivotPoint);
            }
            Invalidate();
        }
        ...
        {
            _isLeftMouseButtonPushed = false;
            _isRightMouseButtonPushed = false;
            offsetXPos = 0;
            offsetYPos = 0;
            _offsetYaw = 0;
            _offsetPitch = 0;
        ...
        {
            if (_isLeftMouseButtonPushed && !cb_CutPoint.Checked)
            {
                float aspect = (float)glControl1.Width / (float)glControl1.Height;
                float deltaX = (e.Location.X - _leftMouseButtonPosition.X) / (float)glControl1.Width;
                float deltaY = (e.Location.Y - _leftMouseButtonPosition.Y) / (float)glControl1.Height;
                _transX += deltaX * _scaling;
                _transY -= deltaY * _scaling / aspect;
                offsetXPos = e.Location.X - _leftMouseButtonPosition.X;
                offsetYPos = e.Location.Y - _leftMouseButtonPosition.Y;
                offsetTransX = (float)(_transX - _previousTransX);
                offsetTransY = (float)(_transY - previousTransY);
                _leftMouseButtonPosition = e.Location;
            }
            if (_isRightMouseButtonPushed && !cb_CutPoint.Checked)
            {
                _angleX += (e.Location.X - _rightMouseButtonPosition.X) / 10.0;
                _angleY += -(e.Location.Y - _rightMouseButtonPosition.Y) / 10.0;
                _offsetYaw = (float)(_angleX - _previousYaw);
                _offsetPitch = (float)(_angleY - _previousPitch);
            ...
            if (cb_CutPoint.Checked)
            {
                ...
                winxyz.Z = 0.3f;
                UnProject(winxyz, ref _mousePosition);
            }
            Invalidate();
        }
        ...
        private void glControl1_MouseClick(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            if (cb_CutPoint.Checked)
            {
                ...
                if (_vertices.Count == 4)
                {
                    ...
                    {
                        _points = CutPoint.FindProjection(_vertices, glControl1, _points);
                        //_pco.FindProjection(_vertices, glControl1, ref _points);
                        _pco.Dispose();
                        _vertices.Clear();
                        _pco = new PointCloudOctree(ref _points, ref _minV, ref _maxV, _colorMapper, _pointCloudColor);
                        GC.Collect();
                        Invalidate();
                        return;
                    }
        ...
        private void openFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (pOpenFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    _resourceManager = new ResourceManager(ref _points);
                    Stopwatch sw = Stopwatch.StartNew();
                    _pco = ReadLas(pOpenFileDialog.FileName);
                    _isFileOpening = true;
                    sw.Stop();
        ...
        private PointCloudOctree ReadLas(string fileName)
        {
            ...
            _resourceManager.ScaleFactor = new Vector3d(xScaleFactor, yScaleFactor, zScaleFactor);
            ...
            _resourceManager.Scale = scale;
            byte classification = 0;
            var coordArray = new double[3];
            var colorArray = new double[3];

            ColorPoint colorPoint = new ColorPoint();
            _points = new List<ColorPoint>(10000);
            _minV.X = (minx - centx) / scale;
            _minV.Y = (miny - centy) / scale;
            _minV.Z = (minz - centz) / scale;
            _maxV.X = (maxx - centx) / scale;
            _maxV.Y = (maxy - centy) / scale;
            _maxV.Z = (maxz - centz) / scale;
            _colorMapper = new PointCloudColorMapper(_minV.Z, _maxV.Z);

            _resourceManager.LasHeader = lazReader.header;
            for (int pointIndex = 0; pointIndex < numberOfPoints; pointIndex++)
            {
                ...
                colorPoint.Color = _colorMapper.MapColor(ColorMode.RGB, null, colorArray);
            ...
            _resourceManager.MinRealCoord = new Vector3d(minx, miny, minz);
            _resourceManager.MaxRealCoord = new Vector3d(maxx, maxy, maxz);
            _resourceManager.CenterCoord = new Vector3d(centx, centy, centz);
            _resourceManager.MinDrawCoord = _minV;
            _resourceManager.MaxDrawCoord = _maxV;
            PointCloudOctree pco = new PointCloudOctree(ref _points, ref _minV, ref _maxV, _colorMapper, _pointCloudColor);
            ...
            ref double objx, ref double objy, ref double objz)
        {
            ...
            finalMatrix.Invert();
            inV.X = winx;
            inV.Y = viewport[3] - winy;
            inV.Z = winz;
            inV.W = 1.0f;
            inV.X = (inV.X - viewport[0]) / viewport[2];
            inV.Y = (inV.Y - viewport[1]) / viewport[3];
            inV.X = inV.X * 2 - 1;
            inV.Y = inV.Y * 2 - 1;
            inV.Z = inV.Z * 2 - 1;
            outV = Vector4d.Transform(inV, finalMatrix);
            if (outV.W == 0.0)
                return (0);
            outV.X /= outV.W;
            outV.Y /= outV.W;
        ...
        {
            if (_isShowOctreeOutline == false)
                _isShowOctreeOutline = true;
            else
                _isShowOctreeOutline = false;
            Invalidate();
        }
        ...
        private void cutToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!_isCuttingPoint)
            {
                cb_CutPoint.BackgroundImage = Properties.Resources.scissors_glyph_icon_free_vector___enabled1;
                _isCuttingPoint = true;
            }
            else
            {
                cb_CutPoint.BackgroundImage = Properties.Resources.scissors_glyph_icon_free_vector;
                _isCuttingPoint = false;
                _vertices = null;
            }
        }
        ...
        private void saveFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            ExportFile.ExportPoints(_points, _resourceManager, progressBar1);
        }
        ...
        private void infoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var header = _resourceManager.LasHeader;
