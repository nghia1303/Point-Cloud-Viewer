...
namespace PointCloudViewer
{
    ...
    //         }
    //     }
        public PointCloudNode(int depth, ref Vector3d minv, ref Vector3d maxv, PointCloudColorMapper colorMapper, string colorType) {
            this.data = new List<ColorPoint>(); 
            this.minCoordinate = minv;
            this.maxCoordinate = maxv;
            this.pointCloudColorMapper = colorMapper;
            this.pointCloudColor = colorType;
            this.depth = depth;
            childNode = new PointCloudNode[8];
            for (int i = 0; i < 8; i++) {
        ...
        private void GenerateDisplayList(string colorType)
        {
            ...
            GL.NewList(displayList, ListMode.Compile);
            GL.Begin(PrimitiveType.Points);
            foreach (var colorPoint in data)
            {
                var color = colorPoint.Color;
                var point = colorPoint.Point;
                var colorArray = new double[3];
                var coordArray = new double[3];
                // Apply color ramp
                if (colorType == "rgb")
        ...
        public void InsertPointToNode(ColorPoint point)
        {
            ...
            // If node has space, add point directly
            if (data.Count > maxPointPerNode)
            {
                data.Add(point);
            ...
            if (childNode[0] == null) // First split
            {
                // Create child nodes
                ...
                {
                    Vector3d minva = new Vector3d(
                        (i & 1) == 0 ? minCoordinate.X : split.X,
                        (i & 2) == 0 ? minCoordinate.Y : split.Y,
                        (i & 4) == 0 ? minCoordinate.Z : split.Z
                    );
                    Vector3d maxva = new Vector3d(
                        (i & 1) == 0 ? split.X : maxCoordinate.X,
                        (i & 2) == 0 ? split.Y : maxCoordinate.Y,
                        (i & 4) == 0 ? split.Z : maxCoordinate.Z
                    );
                    
                    childNode[i] = new PointCloudNode(ref minva, ref  maxva, pointCloudColorMapper, pointCloudColor);
                }

                // Redistribute existing points
                foreach (var existingPoint in data)
                {
                    int index = GetOctantIndex(existingPoint.Point, split);
                    childNode[index].InsertPointToNode(existingPoint);
                }
                
                data.Clear(); // Clear parent node data after redistribution
            }

            // Insert new point into appropriate child
            Vector3d splitPoint = (minCoordinate + maxCoordinate) / 2;
            int childIndex = GetOctantIndex(point.Point, splitPoint);
            childNode[childIndex].InsertPointToNode(point);
        }
    ...
    internal class PointCloudOctree : IDisposable
    {
            ...
            ref Vector3d minv, ref Vector3d maxv, PointCloudColorMapper pointCloudColorMapper, string colorType = "RGB")
        {
            ...
            _root = new PointCloudNode(ref minv, ref maxv, pointCloudColorMapper, colorType);
        }
        public PointCloudOctree(ref Vector3d minv, ref Vector3d maxv, PointCloudColorMapper pointCloudColorMapper, string colorType = "RGB")
        {
            //if (data == null)
            //    return;
            //if (data.Count == 0)
